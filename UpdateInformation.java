...
public class UpdateInformation extends JFrame implements ActionListener{
	...
	UpdateInformation( String meter){
		...
		ImageIcon i1 = new ImageIcon(ClassLoader.getSystemResource("update.jpg.png"));
	...
	public void actionPerformed(ActionEvent ae) {
		if(ae.getSource()==update) {
			...
			try {
				Conn c = new Conn();
				c.s.executeUpdate("update customer set address='"+address+"',city='"+city+"',state='"+state+"',email='"+email+"',phone='"+phone+"'where meter = '"+meter+"'");
				JOptionPane.showMessageDialog(null,"User Information Updaated Successfully");
				setVisible(false);
			}catch(Exception e) {
				e.printStackTrace();
			}
